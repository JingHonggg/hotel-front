  ...
  <div>
    ...
    <div class="container">
      <div class="handle-box">
        ...
        &nbsp;
        <el-input round v-model="costTypes.name" placeholder="请输入消费名称搜索,直接回车即可" class="handle-input mr10" clearable
      ...
      <div class="pagination">
        <el-pagination background layout="total, prev, pager, next" :current-page="costTypes.pageIndex"
          :page-size="costTypes.pageSize" :total="pageTotal" @current-change="handlePageChange"></el-pagination>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      saveCostEdit() {
        ...
        this.$http.post('/addCostType?id=' + this.form.id + '&money=' + this.form.money + '&name=' + this.form.name)
          .then(res => {
            // console.log(res);
      ...
      saveCost() {
        // console.log(this.form);
        this.$http.post('/addCostType?id=0' + '&money=' + this.form.money + '&name=' + this.form.name).then(res => {
          //console.log(res);
          if (res.data.code === 200) {
            //1.提示成功
      ...
      handleSearch() {
        ...
        this.$http.get('/getCostTypeByName?name=' + this.costTypes.name).then(res => {
