  ...
  <div>
    ...
    <div class='container'>
        ...
        <el-table-column label='状态' align='center' width='200'>
          <template slot-scope='scope'>
            ...
            <el-tag v-if='scope.row.state===-1'
              :type="scope.row.state=== 11?'success':(scope.row.state===0?'danger':'')">已退房
            </el-tag>
        ...
        <el-table-column label='操作' width='180' align='center'>
          <template slot-scope='scope'>
            <el-button size='mini' icon='el-icon-edit'
              @click='handleEdit(scope.$index, scope.row,scope.row.name, scope.row.idCard,scope.row.contact)'>编辑
            </el-button>

            <el-button plain size='mini' type='danger' icon='el-icon-delete'
              @click='handleDelete(scope.$index, scope.row,scope.row.idCard)'>删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class='pagination'>
        <el-pagination background layout='total, prev, pager, next' :current-page='guestMsgs.pageIndex'
          :page-size='guestMsgs.pageSize' :total='pageTotal' @current-change='handlePageChange'></el-pagination>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleSearch() {
        ...
        this.$http.get('/getGuestByContact?contact=' + this.search).then(res => {
          if (res.data.code === 200) {
            this.tableData = res.data.data.guestMsgs;
          } else {
            this.$http.get('/getGuestByIdCard?idCard=' + this.search).then(res => {
              if (res.data.code === 200) {
                this.tableData = res.data.data.guestMsgs;
              } else {
                this.$http.get('/getGuestByName?name=' + this.search).then(res => {
                  if (res.data.code === 200) {
                    this.tableData = res.data.data.guestMsgs;
                  } else {
      ...
      handleDelete(index, row, userid) {
        ...
        this.$confirm('确定要删除吗？', '提示', {
          ...
          .then(() => {
            //console.log(userid);
            this.$http.delete('/deleteGuest?idCard=' + userid).then((res) => {
              if (res.data.code === 200) {
