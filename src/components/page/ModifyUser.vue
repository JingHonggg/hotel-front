            ...
            <el-tabs v-model='activeName'>
                <el-tab-pane label='修改信息' name='first'>
                    ...
                    <div style='display:flex;justify-content: center'>
                        <el-button type='primary' style='width: 100px;margin-left: 50px;'
                                   plain @click='infoConfirm' :disabled='role'>提交
                        </el-button>
                    </div>
                </el-tab-pane>
                <el-tab-pane label='修改密码' name='second'>
                    ...
                    <div style='display:flex;justify-content: center'>
                        <el-button type='primary' style='width: 100px;margin-left: 50px;'
                                   plain @click='passwordConfirm' :disabled='role' >提交
                        </el-button>
                    </div>
                </el-tab-pane>
            </el-tabs>
            ...
            this.$confirm('是否确认修改信息？', '提示', {
                ...
                if (this.role === 'admin') {
                   this.$http.post('modifyBackground?newPassword='+this.passwordForm.newPassword).then(res => {
                    ...
                    this.$http.post('/modifyFront?frontId=' + this.userOriginInfo.frontId + '&name=' + this.userOriginInfo.name
                        + '&password=' + this.passwordForm.newPassword + '&phone=' + this.userOriginInfo.phone).then(res => {
                            if (res.status === 200){
                                this.$message.info('修改成功')
                            }else {
                                this.$message.error('修改失败：'+ res.data.msg)
