  ...
  <div>
    ...
    <div class='container'>
      <div class='handle-box'>
        ...
        &nbsp;&nbsp;
        <el-input round v-model='bookMsgs.guestIdCard' placeholder='请输入身份证号搜索,直接回车即可' class='handle-input mr10'
          clearable prefix-icon='el-icon-search' @clear='handleSearch' @keydown.enter.native='handleSearch'>
        </el-input>
      ...
      <el-table :data='tableData' style='width: 100%'>
        ...
        <el-table-column label='状态' align='center' width='200'>
          <template slot-scope='scope'>
            ...
            <el-tag v-if='scope.row.state===11'
              :type="scope.row.state=== 11?'success':(scope.row.state===0?'danger':'')">已入住
            </el-tag>
            <el-tag v-if='scope.row.state===1'
              :type="scope.row.state=== 11?'success':(scope.row.state===0?'danger':'')">已处理
            </el-tag>
            <el-tag v-if='scope.row.state===0'
              :type="scope.row.state=== 11?'success':(scope.row.state===0?'danger':'')">未处理
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label='操作' align='center'>
          <template slot-scope='scope'>
            ...
            <el-button size='mini' type='primary' icon='el-icon-bell' plain
              @click='handleSolve(scope.$index, scope.row,scope.row.rank)'>处理
            </el-button>
            <el-button plain type='success' size='mini' icon='el-icon-key' @click='handleIn(scope.row.id)'>入住
            </el-button>
          </template>
        </el-table-column>
      ...
      <div class='pagination'>
        <el-pagination background layout='total, prev, pager, next' :current-page='bookMsgs.pageIndex'
          :page-size='bookMsgs.pageSize' :total='pageTotal' @current-change='handlePageChange'></el-pagination>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      saveBook() {
        ...
        if (days >= 1) {
          this.$http.post('/addBookMsg?contact=' + this.form.contact + '&fromTime=' + fromTime + '&id=0&idCard=' + this
            .form.idCard + '&name=' + this.form.name + '&toTime=' + toTime + '&rank=' + this.form.rank).then(res => {
            //console.log(res);
            if (res.data.code === 200) {
              //1.提示成功
      ...
      saveEdit() {
        ...
        const days = Math.floor((toTime - fromTime) / (24 * 3600 * 1000));
        if (days >= 1) {
          this.$http.put('/updateBookMsg?fromTime=' + fromTime + '&id=' + this.form.id + '&rank=' + this.form.rank +
            '&toTime=' + toTime).then(res => {
            //console.log(res);
            if (res.data.code === 200) {
              //1.提示成功
      ...
      CheckIn() {
        ...
        const days = Math.floor((toTime - fromTime) / (24 * 3600 * 1000));
        if (days >= 1) {
          this.$http.post('/checkIn?contact=' + this.form.contact + '&fromTime=' + fromTime + '&idCard=' + this.form
            .idCard + '&name=' + this.form.name + '&toTime=' + toTime + '&roomId=' + this.form.resultRoom).then(
            res => {
              //console.log(res);
              if (res.data.code === 200) {
                //1.提示成功
      ...
      handleSolve(index, row) {
        ...
        console.log('formTime: ' + fromTime);
        console.log('toTime: ' + toTime);
        this.$http.get('/getNullRoomsByRank?fromTime=' + fromTime + '&rank=' + this.form.rank + '&toTime=' + toTime)
          .then(res => {
