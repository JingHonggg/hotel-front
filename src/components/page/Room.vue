  ...
  <div>
    ...
    <div class="container">
      <div class="handle-box">
        ...
        &nbsp;&nbsp;
        <el-input round v-model="roomMsgs.roomId" placeholder="请输入房间号搜索,直接回车即可" class="handle-input mr10" clearable
          prefix-icon="el-icon-search" @clear="handleSearch" @keydown.enter.native="handleSearch">
        </el-input>
        <el-button plain round style="float: right" type="primary" icon="el-icon-plus" @click="handBook">添加房间
        </el-button>
      </div>
      ...
      <el-table :data="tableData" style="width: 100%" border>
        ...
        <el-table-column label="房间状态" align="center" width="100">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.state===1" :type="scope.row.state=== 1?'success':(scope.row.state===0?'danger':'')">
              已入住
            </el-tag>
            <el-tag v-if="scope.row.state===-1"
              :type="scope.row.state=== 11?'success':(scope.row.state===-1?'danger':'')">空房
            </el-tag>
            <el-tag v-if="scope.row.state===0"
              :type="scope.row.state=== 11?'success':(scope.row.state===9?'danger':'')">已预订
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button plain size="mini" type="danger" icon="el-icon-delete" v-if="name==='admin'"
              @click="handleDelete(scope.$index, scope.row,scope.row.roomId)">删除
            </el-button>
            <el-button plain size="mini" icon="el-icon-coffee-cup" v-if="name!=='admin'"
              @click="handleService(scope.row.roomId)">消费
            </el-button>
            <el-button plain size="mini" type="success" icon="el-icon-chat-round" v-if="name!=='admin'"
      ...
      <el-dialog title="服务列表" :visible.sync="serviceVisible" width="30%">
        <el-table :data="serviceData" style="width: 100%" border>
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <el-button plain size="mini" icon="el-icon-coffee-cup" @click="sureCost(scope.row.id)">订购
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div class="pagination">
        <el-pagination background layout="total, prev, pager, next" :current-page="roomMsgs.pageIndex"
          :page-size="roomMsgs.pageSize" :total="pageTotal" @current-change="handlePageChange"></el-pagination>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      sureCost(costTypesId) {
        ...
        this.$http.post('/addCost?costTypeId=' + costTypesId + '&id=0&roomId=' + this.costTypes.serviceNeedRooId).then(
          res => {
            console.log(res);
      ...
      //获取所有消费信息
      getAllCostType() {
        this.$http.get('/getAllCostType').then((res) => {
          //console.log(res);
      ...
      surePay() {
        ...
        this.$http.post('/settleCostByRoomId?roomId=' + this.form.roomIdPay).then(res => {
          console.log(res);
          if (res.data.code === 200) {
            //1.提示成功
      ...
      inspect(roomId) {
        ...
        this.$http.post('/getCostByRoomId?roomId=' + roomId).then(res => {
          //console.log(res);
      ...
      checkOut(checkroomId) {
        ...
        this.$http.post('/checkOut?roomId=' + checkroomId).then(res => {
          console.log(res);
          if (res.data.code === 200) {
            //1.提示成功
      ...
      saveRoomEdit() {
        ...
        if (this.form.maxNum > 0 && this.form.earnest > 0 && this.form.rent > 0 && this.form.size > 0) {
          this.$http.post('/addRoom?earnest=' + this.form.earnest + '&roomId=' + this.form.roomId + '&maxNum=' + this
            .form.maxNum + '&rank=' + this.form.rank + '&rent=' + this.form.rent + '&size=' + this.form.size +
            '&position=' + this.form.position).then(res => {
            //console.log(res);
            if (res.data.code === 200) {
              //1.提示成功
      ...
      // 搜索功能
      handleSearch() {
        this.$http.get('/getRoomById?id=' + this.roomMsgs.roomId).then(res => {
      ...
      //获取已经预定的房间
      getHasBookedRoom() {
        this.$http.get('/getHasBookedRoom').then(res => {
          if (res.data.data === null) {
      ...
      //获取已经入住的房间
      getHasCheckedRoom() {
        this.$http.get('/getHasCheckedRoom').then(res => {
          if (res.data.data === null) {
